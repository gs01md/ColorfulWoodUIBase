//
//  CWUBCell_TitleLeft_InputRight_NumRight.m
//  ColorfulWoodUIBase
//
...
@interface CWUBCell_TitleLeft_InputRight_NumRight()<
UITextViewDelegate
>
...
@implementation CWUBCell_TitleLeft_InputRight_NumRight

- (id)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier model:(CWUBCell_TitleLeft_InputRight_NumRight_Model*)model{
...
- (void)func_updateConsrtains{
    ...
    [_m_lbl_numRight mas_remakeConstraints:^(MASConstraintMaker *make) {
         ...
         * 是否显示，如果不限制输入字数，则不显示
         */
        if (self.m_model.m_title_numRight.m_isHidden) {
            make.top.equalTo(self.m_textfield_center);
            make.width.mas_equalTo(1);
            make.right.equalTo(self);
        } else {
            make.top.equalTo(self.m_textfield_center);
            make.width.mas_equalTo([self.m_model.m_title_numRight interface_setWidthByContent]);
            make.right.equalTo(self).offset(-self.m_model.m_title_numRight.m_margin_right);
        }
    ...
    [_m_textfield_center mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.bottom.equalTo(self.m_img_sep.mas_top).offset(-self.m_model.m_input_center.m_margin_bottom);
        make.left.equalTo(self.m_lbl_left.mas_right).offset(self.m_model.m_input_center.m_margin_left);
        make.right.equalTo(self.m_lbl_numRight.mas_left).offset(-self.m_model.m_input_center.m_margin_right);
        if (textViewHeight>0) {
    ...
    [self.m_img_sep mas_remakeConstraints:^(MASConstraintMaker *make) {
        ...
        make.top.equalTo(self.m_textfield_center.mas_bottom).offset(self.m_model.m_bottomLineInfo.m_margin_top);
    }];
}

-(CWUBCell_TitleLeft_InputRight_NumRight_Model*) m_model{

    if (!_m_model) {
        _m_model = [CWUBCell_TitleLeft_InputRight_NumRight_Model new];
    }
...
- (CWUBLabelWithModel *)m_lbl_numRight{
    ...
    if (!_m_lbl_numRight) {
        _m_lbl_numRight = [[CWUBLabelWithModel alloc] initWithModel:self.m_model.m_title_numRight];
...
- (void) interface_updateWithModel:(CWUBCell_TitleLeft_InputRight_NumRight_Model*)model{
    ...
    [self.m_lbl_numRight interface_update:model.m_title_numRight];
    if (self.m_model.m_bottomLineInfo.m_color) {
        self.m_img_sep.backgroundColor = self.m_model.m_bottomLineInfo.m_color;
    }else{
        self.m_img_sep.backgroundColor = [UIColor clearColor];
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text{
    ...
    if (self.m_model.m_input_center.m_limitInputNum>0) {
        ...
        if (new_text_str.length>self.m_model.m_input_center.m_limitInputNum) {
            ...
            self.m_model.m_title_numRight.m_text = self.m_lbl_numRight.text;
        }
